...
### 벡터란(vector)?
벡터는 STL에서 가장 흔히 쓰이는 컨테이너 중 하나로써 연속적인 메모리 주소를 가지고 있어 인덱싱을 지원하고 속도 또한 빠르다.
하지만 연속적인 주소를 가지고 있기 때문에 삭제와 삽입에 필요한 비용이 많이 들어간다는 단점이 있다.
...
vec2.pop_back();
// **emplace() : 원하는 곳 뒤에 () 안에 있는 데이터를 컨테이너 안에서 생성자를 통해 객체를 생성 후 넣는다. 
// (복사비용이 X) but 포인터를 을때는 의미 X
